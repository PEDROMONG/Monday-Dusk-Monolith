...
class FreeplayState extends MusicBeatState
{
	...
	var color:FlxColor;
	var artistNames:Array<String> = CoolUtil.coolTextFile(Paths.txt('data/artistNames'));
	...
	public static function getCharacterOffsets(character:Character) {
		switch (character.curCharacter)
		{
			...
			case 'dad':
				character.x += -140;
				character.y += -150;
				character.scale.x = 0.85;
				character.scale.y = 0.85;
			case 'dad-saving':
				character.x += -150;
				character.y += -140;
				character.scale.x = 0.85;
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('freeplayBGTrue'));
		...
		character = new Character(FlxG.width/2, 75, songs[curSelected].songCharacter, flipX = true);
		character.scrollFactor.set(0, 0);
		getCharacterOffsets(character);
		character.color = FlxColor.BLACK;
		add(character);

		var bg2:FlxSprite = new FlxSprite().loadGraphic(Paths.image('freeplayBGUntrue'));
		if(FlxG.save.data.antialiasing)
			{
		...
		scoreText = new FlxText(FlxG.width * 0.7, 620, 0, "", 32);
		// scoreText.autoSize = false;
		scoreText.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, RIGHT);
		// scoreText.alignment = RIGHT;

		var scoreBG:FlxSprite = new FlxSprite(scoreText.x - 6, 615).makeGraphic(Std.int(FlxG.width * 0.35), 105, 0xFF000000);
		scoreBG.alpha = 0.6;
		add(scoreBG);
		...
		txtArtistTitle = new FlxText(FlxG.width * 0.7, 10, 0, "", 32);
		txtArtistTitle.setFormat("VCR OSD Mono", 32, FlxColor.WHITE, RIGHT);
	...
	override function update(elapsed:Float)
	{
		...
		txtArtistTitle.text = artistNames[curSelected].toUpperCase();
		txtArtistTitle.x = FlxG.width - (txtArtistTitle.width + 10);
		...
		if (FlxG.mouse.overlaps(diffText))
		{
