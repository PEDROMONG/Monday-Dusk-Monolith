...
class Alphabet extends FlxSpriteGroup
{
	...
	public function new(x:Float, y:Float, text:String = "", ?bold:Bool = false, ?typed:Bool = false, ?stepped:Bool = true, ?alignX:Float = 90, ?alignY:Float = 0.48, ?drawHypens:Bool = false, ?wheel:Bool = false, xScale:Float = 1, yScale:Float = 1)
	{
		super(x, y);
	...
	public function reType(text, xScale:Float = 1, yScale:Float = 1)
		{
			...
			var wackyThing:Array<FlxSprite> = [];
			forEach(function (sprite:FlxSprite) {
				wackyThing.push(sprite);
			});
			clear();
			for( kid in wackyThing ) {
				kid.destroy();
			}
	...
	override function update(elapsed:Float)
	{
		if (isMenuItem)
		{
			...
			switch (itemType) {
				case "Classic":
					x = FlxMath.lerp(x, (targetY * 20) + groupX, 0.16 / (CoolUtil.fps / 24));
					y = FlxMath.lerp(y, (scaledY * 120) + (FlxG.height * groupY), 0.16 / (CoolUtil.fps / 24));
				case "Vertical":
					y = FlxMath.lerp(y, (scaledY * 120) + (FlxG.height * 0.5), 0.16 / (CoolUtil.fps / 24));
				...
				case "C-Shape":
					...
					y = FlxMath.lerp(y, (scaledY * 65) + (FlxG.height * 0.39), 0.16 / (CoolUtil.fps / 24));

					x = FlxMath.lerp(x, Math.exp(scaledY * 0.8) * 70 + (FlxG.width * 0.1), 0.16 / (CoolUtil.fps / 24));
					if (scaledY < 0)
						x = FlxMath.lerp(x, Math.exp(scaledY * -0.8) * 70 + (FlxG.width * 0.1), 0.16 / (CoolUtil.fps / 24));
				...
				case "D-Shape":
					y = FlxMath.lerp(y, (scaledY * 90) + (FlxG.height * 0.45), 0.16 / (CoolUtil.fps / 24));

					x = FlxMath.lerp(x, Math.exp(Math.abs(scaledY * 0.8)) * -70 + (FlxG.width * 0.35), 0.16 / (CoolUtil.fps / 24));
