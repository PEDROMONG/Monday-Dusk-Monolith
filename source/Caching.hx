...
class Caching extends MusicBeatState
{
	...
	override function create()
	{
		...
		toBeDone = Lambda.count(images) + Lambda.count(music) + Lambda.count(week2);
	...
	function cache()
	{
		...
		for (i in week2)
			{
				var replaced = i.replace(".png","");
				var data:BitmapData = BitmapData.fromFile("assets/week2/images/" + i);
				trace('id ' + replaced + ' file - assets/week2/images/' + i + ' ${data.width}');
				var graph = FlxGraphic.fromBitmapData(data);
				graph.persist = true;
				graph.destroyOnNoUse = false;
				bitmapData.set(replaced,graph);
